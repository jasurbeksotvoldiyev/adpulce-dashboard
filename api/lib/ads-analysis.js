...
const { getCreativeStats } = require('./fb-ads');
...
async function analyzeProject(project) {
  const ads = await getCreativeStats(project.fbAccountId, 'today');
...
function buildAnalysisText(result) {
  ...
  const parts = [`<b>${project.name}</b> — kunlik kreativ tahlili`];
...
Ishlamagan kreativlar:
${notWorking.map(a => `- ${a.adName}: sarf ${a.spend} ${project.currency}, lead ${a.leads}, narxi ${a.cpl ?? '—'}`).join('\n')}
Ishlagan kreativlar soni: ${working.length}, hali erta bo'lganlar: ${early.length}`;
