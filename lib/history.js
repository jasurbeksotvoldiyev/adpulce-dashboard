...
async function getProjectCreatives(project) {
  ...
  try {
    ...
    const list = file && file.date === tashkentDay(0) && file.ads ? file.ads[key] : null;
  ...
  return { level: 'campaign', items: await getProjectCampaigns(project, 'today') };
}

module.exports = { getProjectStats, getProjectCampaigns, getProjectCreatives, tashkentDay };
